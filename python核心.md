...
# type即使对象也是类,以下形成循环引用
# type的父类是object
# object的类型又是type,即object是由type生成
# 原因：object是最顶层的基类，而type是类，所以被object管. type管对象，而object既可以说是类，也可以说是对象所以被type管
...
# isinstance会去检查继承类，所以arg2是父类类型，也为True
...
- dict的查找性能远远大于list
- 在list随着list数据的增大，查找事件会增大
- 在dict中查找元素不会随着dict的增大而增大
- dict的key是由hash实现的，所以不可hash的对象不可以作为dict的key
- set和dict的实现原理是一样的，所以dict的key和set的值都必须是可hash的
- 不可变对象都是可hash的，比如str, frozenset, tuple。
- 自己实现的类 ，实现 __hash双下划线 方法，就是一个可hash的对象
- dict的内存花销大，但是查询速度快，自定义的对象或者python的内部对象，都是dict包装的
...
__del__ 魔法函数，当python解释器回收对象时，会调用__del__魔法方法
'''
a = 1       # 计数器为1
b = 1       # 计数器为2
c = 1       # 计数器为3
...
if __name__ == '__main__':
    u = User("nice")
    # 查找不到age,则会去__getattr__方法，getattr方法返回一个'not found attr'
    print(u.age)
```
...
```python
'''
	只要实现 __get__  __set__  __delete__ 任意一个方法的类，它就是属性描述符
'''
import numbers
class IntField:
...
python中类的实例化(注意不是对象的实例化，而是类对象的实例化)过程:
    ...
    2.如果找不到metaclass则去父类中找metaclass
